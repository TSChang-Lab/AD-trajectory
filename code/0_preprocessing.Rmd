...
patient_level = AD_merged %>% left_join(AD_diagnosis) %>% 
  ...
  filter(record_length > 0)
dim(patient_level) # dim = (6475,5)
...
diagnosis_level = AD_merged %>% right_join(patient_level) %>% 
  ...
  filter((FirstDiagnosisDate < AdDiagnosisDate) & 
           (FirstDiagnosisDate >= AdDiagnosisDate - years(5))) %>% 
  select(PatientID, Sex, BirthDate, race_ethnicity, record_length, AdDiagnosisDate, 
...
dim(diagnosis_level) # dim = (145323,12)
diagnosis_level2 =  AD_merged %>% right_join(patient_level) %>% 
  filter((EncounterDate < AdDiagnosisDate) & 
           (EncounterDate >= AdDiagnosisDate - years(5))) %>% 
  group_by(PatientID) %>% mutate(n_record = length(unique(EncounterID))) %>% ungroup() %>% 
  select(PatientID, n_record)
